...
export const javascriptInterviewQuestions: Record<string, InterviewQuestion[]> =
  {
    'adding-removing-methods': [
      ...
      },
      {
        ...
        answer:
          '`splice()` returns an array containing the elements that were removed (an empty array if none were removed). Its signature is `array.splice(start, deleteCount, item1, item2, ...)`:\n\n- **start** — the index to begin at (negative values count from the end)\n- **deleteCount** — how many elements to remove from that index\n- **items** — any additional arguments are new elements inserted at that same position\n\n```js\nconst arr = [1, 2, 3, 4, 5];\narr.splice(1, 2, "a", "b"); // removes 2,3 and inserts "a","b"\n// arr is now [1, "a", "b", 4, 5]\n```',
      ...
      },
      {
        ...
        answer:
          'These array methods **mutate** the original array in place:\n\n- `push()`\n- `pop()`\n- `shift()`\n- `unshift()`\n- `splice()`\n\nTo get an equivalent result without mutating, use non-mutating alternatives instead:\n\n- `slice()` to copy out a portion\n- `concat()` or the spread operator `[...arr]` to add elements into a new array\n- `filter()` to remove elements into a new array\n- The newer ES2023 methods `toSpliced()`, `toSorted()`, and `toReversed()`, which return a new array and leave the original untouched',
        difficulty: 'intermediate',
      },
      {
    ...
    'array-destructuring': [
      ...
      },
      {
        ...
        answer:
          'You leave an empty slot in the destructuring pattern by using a comma with nothing between it and the next comma.\n\n```js\nconst [a, , c] = [1, 2, 3];\n// a === 1, c === 3 — the second element is skipped entirely\n```',
        difficulty: 'beginner',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Yes, a function parameter list can contain a destructuring pattern directly.\n\n```js\nfunction formatName([first, last]) {\n  return `${first} ${last}`;\n}\n```\n\nThis is useful because it unpacks an array argument into named variables immediately at the point of declaration, avoiding an extra line inside the function body to do the same unpacking manually.',
        difficulty: 'intermediate',
      },
    ],
    ...
    arrays: [
      {
        ...
        answer:
          'All three iterate over an array, but they differ in purpose and return value.\n\n- **forEach** — runs a function for each element and returns `undefined`. It is for side effects, not building a new array.\n- **map** — returns a new array of the same length containing the transformed result of each element.\n- **filter** — returns a new array containing only the elements for which the callback returns `true`.\n\n`map` and `filter` are non-mutating and chainable, while `forEach` is typically used when you just need to do something per element.',
        difficulty: 'beginner',
      },
      {
        question: 'How does reduce work, and what is a real use case for it?',
        answer:
          '`reduce` boils an array down to a single accumulated value by running a reducer callback for each element, passing along an accumulator. The callback receives the accumulator and the current element and returns the updated accumulator, and you can supply an initial value as the second argument.\n\n```js\nconst total = [10, 20, 30].reduce((sum, n) => sum + n, 0);\n// total === 60\n```\n\nCommon uses include summing numbers, counting occurrences into an object, grouping items by a key, or flattening arrays — anywhere you need to fold a list into one result.',
        difficulty: 'intermediate',
      },
      {
      ...
      },
      {
        ...
        answer:
          'The most concise modern way is to pass the array to a `Set`, which stores only unique values, then spread it back into an array.\n\n```js\nconst unique = [...new Set(arr)];\n```\n\nThis works for primitives because `Set` compares them by identity. For arrays of objects, where each object is a distinct reference, you instead deduplicate by a chosen key, for example by tracking seen keys in a `Set` while filtering, or by building a `Map` keyed on the property that defines uniqueness.',
        difficulty: 'intermediate',
      },
      {
      ...
      },
      {
        ...
        answer: '`0` — array indexes in JavaScript are zero-based.',
        difficulty: 'beginner',
      },
      {
    ...
    'async-await': [
      {
        ...
        answer:
          '`forEach` does not await the async callback you pass it — it calls each callback and moves on without waiting for the returned promise, so surrounding code continues before the async work finishes and errors inside the callback are not caught by an outer `try/catch`.\n\n- To process items **sequentially**, use a plain `for...of` loop where `await` genuinely pauses each iteration.\n- To run them **in parallel** and wait for all, map the array to an array of promises and `await Promise.all()` on it.\n\n```js\n// sequential\nfor (const url of urls) {\n  await fetch(url);\n}\n\n// parallel\nawait Promise.all(urls.map((url) => fetch(url)));\n```',
        difficulty: 'advanced',
      },
      {
        question:
      ...
      },
      {
        ...
        answer:
          'You start all the operations first so they run concurrently, collect their promises into an array, then await `Promise.allSettled` on that array. Unlike `Promise.all`, which rejects as soon as any single promise rejects, `allSettled` waits for every promise and returns an array describing each outcome as either fulfilled with a value or rejected with a reason.\n\n```js\nconst results = await Promise.allSettled([fetchA(), fetchB(), fetchC()]);\nresults.forEach((r) => {\n  if (r.status === "fulfilled") console.log(r.value);\n  else console.error(r.reason);\n});\n```\n\nYou then iterate the results and handle successes and failures individually, which is ideal when one failure should not discard the other successful results.',
        difficulty: 'intermediate',
      },
      {
        question:
      ...
      },
      {
        ...
        answer:
          'You wrap the `await` expressions in a `try/catch` block, just as you would for synchronous code.\n\n```js\ntry {\n  const data = await fetchData();\n} catch (err) {\n  console.error(err);\n}\n```\n\nIf the awaited promise rejects, execution jumps to the `catch` block, letting you handle the error in the same familiar shape used for regular thrown errors.',
        difficulty: 'beginner',
      },
      {
        question:
    ...
    'class-features': [
      ...
      },
      {
        ...
        answer:
          'You declare a private field by prefixing its name with `#`, declared in the class body.\n\n```js\nclass Account {\n  #balance = 0;\n  deposit(amount) {\n    this.#balance += amount;\n  }\n}\n```\n\nUnlike a leading-underscore convention, which is purely a naming signal that JavaScript does not enforce, a `#` field is enforced by the language itself — accessing it from outside the class throws a `SyntaxError`, making it genuinely inaccessible rather than just discouraged.',
        difficulty: 'intermediate',
      },
      {
        question:
    ...
    closures: [
      {
        ...
        answer:
          'You create an outer function that declares a boolean flag and a stored result in its scope, then returns an inner function that closes over them. The first call sets the flag to `true`, runs the real work, and caches the result; every later call sees the flag is already `true` and simply returns the cached result without re-running the work.\n\n```js\nfunction once(fn) {\n  let called = false;\n  let result;\n  return (...args) => {\n    if (!called) {\n      called = true;\n      result = fn(...args);\n    }\n    return result;\n  };\n}\n```\n\nThis pattern is common for one-time initialization or making sure a handler like a payment submit cannot fire twice, and it works purely because the inner function keeps a live reference to the flag between calls.',
        difficulty: 'advanced',
      },
      {
        question:
        ...
        answer:
          'You write a `makeCounter` function that declares `let count = 0` and returns an object of functions such as `increment`, `decrement`, and `value`, each closing over the same `count` variable.\n\n```js\nfunction makeCounter() {\n  let count = 0;\n  return {\n    increment: () => ++count,\n    decrement: () => --count,\n    value: () => count,\n  };\n}\n```\n\nBecause `count` is a local variable of `makeCounter` and is never assigned as a property on the returned object, nothing outside has a reference to it — callers can only change it through the exposed methods. This gives true data privacy: the count is encapsulated inside the closure and can only be mutated in the controlled ways the returned methods allow.',
        difficulty: 'intermediate',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Currying transforms a function that takes several arguments into a sequence of functions that each take one argument. Each returned function closes over the arguments supplied so far and waits for the next one, only running the real computation once all arguments have been collected.\n\n```js\nconst add = (a) => (b) => (c) => a + b + c;\nadd(1)(2)(3); // 6\n```\n\nClosures are what let each stage remember the earlier arguments, so a curried `add(a)(b)(c)` works because the innermost function still has access to `a` and `b` through the closures formed at each step. Currying is useful for creating specialized, reusable functions and for partial application.',
        difficulty: 'advanced',
      },
      {
      ...
      },
      {
        ...
        answer:
          '`var` is function-scoped, so a `for` loop declared with `var` creates only one shared variable across all iterations; by the time any `setTimeout` callback runs, the loop has finished and every callback sees the same final value.\n\n```js\nfor (var i = 0; i < 3; i++) {\n  setTimeout(() => console.log(i), 0); // logs 3, 3, 3\n}\nfor (let j = 0; j < 3; j++) {\n  setTimeout(() => console.log(j), 0); // logs 0, 1, 2\n}\n```\n\n`let` is block-scoped, so each iteration gets its own fresh binding, and each callback closes over its own separate variable, producing the expected sequence of values.',
        difficulty: 'intermediate',
      },
      {
    ...
    'data-types-in-javascript': [
      {
        ...
        answer:
          '`typeof` is not enough because `typeof []` and `typeof null` both return `"object"`.\n\n- To detect arrays, use `Array.isArray(value)`.\n- To check specifically for `null`, compare with `value === null`.\n- For a precise type tag on any value, use `Object.prototype.toString.call(value)`, which returns strings like `[object Array]`, `[object Date]`, or `[object Null]`, giving you a dependable way to distinguish object subtypes that `typeof` cannot.',
        difficulty: 'intermediate',
      },
      {
        question:
    ...
    'date-and-time': [
      ...
      },
      {
        ...
        answer:
          'You can either add the equivalent number of milliseconds to the timestamp and construct a new `Date` from the result, or mutate an existing `Date` in place with `setDate()`, which correctly rolls over into the next month or year when needed.\n\n```js\nconst date = new Date();\ndate.setDate(date.getDate() + 7); // one week from now\n```',
        difficulty: 'intermediate',
      },
    ],
    ...
    dom: [
      ...
      },
      {
        ...
        answer:
          'Event delegation is the practice of attaching a single event listener to a common ancestor instead of to many individual child elements, then using `event.target` to determine which child was actually interacted with. It relies on event bubbling.\n\nIt is useful because it reduces the number of listeners, improving memory and performance, and it automatically handles elements added to the DOM later, which a directly bound listener would miss.',
        difficulty: 'intermediate',
      },
      {
        question:
        ...
        answer:
          "They control different things.\n\n- **event.preventDefault()** — stops the browser's default action for an event, for example preventing a form from submitting and reloading the page, or a link from navigating.\n- **event.stopPropagation()** — stops the event from continuing to travel through the DOM to listeners on other elements, but does not affect the default action.\n\nThey are independent, and you sometimes use both together.",
        difficulty: 'intermediate',
      },
      {
    ...
    'error-handling': [
      {
        ...
        answer:
          'A synchronous `try/catch` only catches errors thrown during the synchronous execution of the `try` block. Errors from asynchronous callbacks scheduled later, such as inside `setTimeout`, are not caught because that code runs after the `try/catch` has already finished.\n\nFor promise-based async work, you either chain a `.catch` on the promise or, with `async/await`, wrap the `await` expressions in `try/catch` — which works because `await` turns a rejected promise back into a thrown error at that point in the async function.',
        difficulty: 'intermediate',
      },
      {
      ...
      },
      {
        ...
        answer:
          'You create a class that extends the built-in `Error` class, call `super(message)` inside the constructor to properly set up the message and stack trace, and then set `this.name` to a descriptive string.\n\n```js\nclass ValidationError extends Error {\n  constructor(message) {\n    super(message);\n    this.name = "ValidationError";\n  }\n}\n```\n\nInstances of this class remain `instanceof Error` while also being `instanceof` your custom class, so you can catch and distinguish them.',
        difficulty: 'intermediate',
      },
      {
    ...
    'event-loop': [
      {
        ...
        answer:
          'The output is:\n\n```js\n1\n4\n3\n2\n```\n\nThe synchronous logs `1` and `4` run first because they execute directly on the call stack. When the stack empties, the event loop drains the entire microtask queue before any macrotask, so the promise callback logging `3` runs next. Only then does the `setTimeout` macrotask logging `2` run. The key rule is that **all microtasks run before the next macrotask**.',
        difficulty: 'advanced',
      },
      {
    ...
    'fetch-api': [
      ...
      },
      {
        ...
        answer:
          'You create an `AbortController`, pass its `.signal` property into the `fetch()` options object, and call `controller.abort()` whenever you want to cancel the request.\n\n```js\nconst controller = new AbortController();\nfetch(url, { signal: controller.signal });\ncontroller.abort();\n```\n\nThe pending `fetch()` promise then rejects with an error whose name is `"AbortError"`, which you can catch and handle separately from other failures.',
        difficulty: 'advanced',
      },
    ],
    ...
    'generators-iterators': [
      ...
      },
      {
        ...
        answer:
          '`function*` declares a generator function. Calling it does not run its body immediately — instead it returns a generator object (an iterator and iterable) without executing any code yet.\n\n```js\nfunction* range() {\n  yield 1;\n  yield 2;\n  yield 3;\n}\nconst it = range(); // body has not run yet\nit.next(); // { value: 1, done: false }\n```\n\nThe body only runs, up to the next `yield`, when `.next()` is called on that generator object.',
        difficulty: 'intermediate',
      },
      {
    ...
    modules: [
      ...
      },
      {
        ...
        answer:
          'You use the namespace import syntax, which gathers all of that module\'s named exports into a single object, with each export accessible as a property on it.\n\n```js\nimport * as utils from "./module.js";\nutils.formatDate(...);\n```',
        difficulty: 'beginner',
      },
      {
    ...
    'object-destructuring': [
      ...
      },
      {
        ...
        answer:
          'You use a colon inside the destructuring pattern.\n\n```js\nconst { originalKey: newVariableName } = obj;\n```\n\nThe name before the colon must match the actual property key on the object, while the name after the colon is the new local variable name that will hold the value.',
        difficulty: 'beginner',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Destructuring properties from `null` or `undefined` throws a `TypeError` because there is no object to read from. A common guard is to give the whole parameter a default empty object.\n\n```js\nfunction greet({ name } = {}) {}\n```\n\nSo that calling `greet()` without an argument destructures from `{}` instead of `undefined`.',
        difficulty: 'intermediate',
      },
    ],
    ...
    promises: [
      ...
      },
      {
        ...
        answer:
          'A Promise starts in the **pending** state, meaning the operation has not finished yet. It then transitions to either:\n\n- **fulfilled** — completed successfully and has a resulting value\n- **rejected** — failed and has a reason\n\nOnce a promise settles into fulfilled or rejected, its state cannot change again.',
        difficulty: 'beginner',
      },
      {
    ...
    'rest-parameters': [
      {
        ...
        answer:
          "Rest parameters, written as `...name` at the end of a function's parameter list, collect every remaining argument passed to the function call into a single real array.\n\n```js\nfunction sum(...numbers) {\n  return numbers.reduce((a, b) => a + b, 0);\n}\nsum(1, 2, 3); // numbers === [1, 2, 3]\n```",
        difficulty: 'beginner',
      },
      {
